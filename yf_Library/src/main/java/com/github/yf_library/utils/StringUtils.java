...
public class StringUtils {
	...
	/**
	 * 灏嗕紶鍏ョ殑瀵硅薄浠ュ瓧绗︿覆鐨勫舰寮忥紝鐢ㄥ紩鍙峰寘瑁硅捣鏉�
	 * 
	 * @param input
	 * @return
	 */
	public static String surroundWithQuotationMark(String input) {
		return surroundWithMark(input, "\"");
	}

	/**
	 * 灏嗕紶鍏ョ殑瀵硅薄浠ュ瓧绗︿覆鐨勫舰寮忥紝鐢╩ark鍖呰９璧锋潵
	 * 
	 * @param input
	 * @return
	 */
	public static String surroundWithMark(String input, String mark) {
		if (input != null) {
			input = trimWithMark(input, mark);
	...
	/**
	 * 灏嗕紶鍏ョ殑瀵硅薄浠ュ瓧绗︿覆鐨勫舰寮忥紝闄ゅ幓棣栧熬鐨刴ark
	 * 
	 * @param mark
	 *            
	 * @param input
	 * @return
	 */
	public static String trimWithMark(String input, String mark, int type) {
		if (isNotBlank(input) && mark != null) {
			if (type < 0) {
				while (input.startsWith(mark)) {
					input = input.substring(mark.length(), input.length());
				}
			} else if (type == 0) {
				while (input.startsWith(mark)) {
					input = input.substring(mark.length(), input.length() - 1);
				}

				while (input.endsWith(mark)) {
					int index = input.lastIndexOf(mark);
	 ...
	 * @return
	 */
	public static String trimWithMark(String input, String mark) {
		return trimWithMark(input, mark, 0);
	}
	 ...
	 * @return
	 */
	public static List<String> splitByMark(String manager, String mark) {
		if (StringUtils.isNotBlank(manager)) {
			...
			String[] strs = manager.split(mark);
			if (strs != null) {
    ...
    public static void main(String[] args) {
		System.out.println(StringUtils.isMobile("18082286602"));
	}
}